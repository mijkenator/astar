...
element_in_set( #peak{x=X, y=Y} = _E, Set) -> btrie:is_key(<<X,Y>>, Set).

-spec remove_from_set(peak(), mset())      -> mset().
remove_from_set(#peak{x=X, y=Y} = _E, Set) -> btrie:erase(<<X,Y>>, Set).

-spec add_to_set(peak(), mset())           -> mset().
add_to_set(     #peak{x=X, y=Y} = E, Set)  -> btrie:append(<<X,Y>>, E, Set).

-spec filter_not_in_set(mset(), [{integer(), integer()}]) -> [{integer(), integer()}].
filter_not_in_set(Set, PeakList) -> 
    lists:filter(fun({X,Y})-> false =:= btrie:is_key(<<X,Y>>, Set) end, PeakList).

-spec get_peak_withmin_f(mset()) -> peak() | noway.
get_peak_withmin_f(Set) ->
    case btrie:to_list(Set) of
        [] -> noway;
        [{_,[H]}|T] -> gpwfi(T, H)
    end.
